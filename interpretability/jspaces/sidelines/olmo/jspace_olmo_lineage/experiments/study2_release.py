"""Render, publish, and verify the partial-safe OLMo Study-2 handoff."""
...
def registry_prefix_record(path: str | Path = EVENTS) -> dict:
    ...
    try:
        path_label = str(source.resolve().relative_to(REPO_ROOT.resolve()))
    except ValueError:
    ...
    return {
        ...
        "live_outputs": sum(
            len(record["effective_metadata"].get("outputs") or []) for record in live
        ),
...
def admitted_evidence_records(config: Mapping) -> list[dict]:
    ...
    for evidence_id in identifiers:
        ...
        outputs = []
        for row in effective.get("outputs") or []:
            path = Path(row["path"])
            if not path.is_file():
                raise FileNotFoundError(path)
            actual = file_sha256(path)
            if actual != row["sha256"] or path.stat().st_size != int(row["bytes"]):
                raise ValueError(f"admitted output hash drift: {path}")
        ...
        records.append(
            {
                ...
                "decisions": {
                    key: effective[key] for key in DECISION_KEYS if key in effective
                },
                "outputs": outputs,
            }
        )
...
def _render_markdown(envelope: Mapping) -> str:
    ...
    lines = [
        ...
        "",
        (
            "This bundle is partial-safe: every mandatory GPU stage completed, "
            "while gated, unavailable, conditional, and unopened items remain "
            "explicitly partial. Development/methods tiers are preserved."
        ),
        "",
        "## Scientific disposition",
        "",
        f"- Stage route: `{summary['stage_route']}`; effects are missing, not zero.",
        f"- H6 route: `{summary['h6_route']}`; no L24/L32/L40 cell passes.",
        (
            "- Think passes only the L56, epsilon-0.10 late anchor (12/12); "
            "Base reaches 9/12 there."
        ),
        (
            "- Registered-dose coverage is unavailable because exact site-dose "
            "records are absent; it is not zero."
        ),
        (
            f"- OLMo-pair power is "
            f"{summary['pair_power_at_frozen_sesoi']:.4f} at "
            f"{summary['pair_shared_capable_families']} families versus target "
            f"{summary['pair_power_target']:.2f}; "
            f"{summary['pair_minimum_powered_families']} families are required."
        ),
...
def render(config_path: str | Path) -> dict:
    ...
    artifacts = source_artifact_records(config)
    admissions = admitted_evidence_records(config)
...
def verify_bundle_source(config_path: str | Path) -> dict:
    ...
    if not commit_reachable(source_commit):
        raise ValueError("bundle render commit is not reachable")
    if not _generation_protocol_unchanged(source_commit, config_path):
        raise ValueError("release protocol changed after rendering")
    ...
    if (
        ...
        or snapshot.stat().st_size != payload["registry_snapshot"]["bytes"]
    ):
    ...
    for required in (
        "effects are missing, not zero",
        "Registered-dose coverage is unavailable",
        "No intervention is opened",
    ):
...
def _publish_pairs(config: Mapping) -> list[tuple[Path, Path]]:
    pairs = [
        (
            _repo_path(row["repo_path"]),
            resolve_uri(row["target_uri"], must_exist=False),
        )
    ...
    pairs.extend(
        [
            (
                ...
                resolve_uri(
                    config["outputs"]["drive_registry_prefix"], must_exist=False
                ),
            ),
            (
                repo["bundle_json"],
            ...
            (
                ...
                resolve_uri(
                    config["outputs"]["drive_bundle_markdown"], must_exist=False
                ),
            ),
        ]
    )
...
def publish(config_path: str | Path) -> dict:
    ...
    bundle_json = _repo_outputs(config)["bundle_json"]
    bundle = _load_envelope(bundle_json)
    payload = bundle["payload"]
    event = create(
        config["evidence_id"],
        tier=config["tier"],
        what=(
            "Partial-safe terminal OLMo Study-2 import bundle: mandatory stage "
            "wedge and H6 complete; gated, unavailable, conditional, and "
            "underpowered items remain explicit; no intervention opened."
        ),
        command=(
            "python -m jspace_olmo_lineage.experiments.study2_release "
            f"--config {config_path} --publish"
        ),
        outputs=output_paths,
        inputs={
            "config": file_sha256(config_path),
            "bundle_json": file_sha256(bundle_json),
        ...
        stage_route=config["result_summary"]["stage_route"],
        h6_route=config["result_summary"]["h6_route"],
        registered_dose_coverage="unavailable-not-zero",
        bank_w_pair_route="not-powered-at-current-support",
        claim_boundary=config["claim_boundary"],
    )
    return {
        "ok": True,
...
def verify_published(config_path: str | Path) -> dict:
    ...
    if effective.get("verdict") != "complete-mandatory-partial-conditional":
    ...
    return {
        ...
        "bundle_id": config["bundle_id"],
        "evidence_id": config["evidence_id"],
        "release_event_commit": effective["code_commit"],
        "bundle_source_commit": source["source_commit"],
        "bundle_json_sha256": file_sha256(_repo_outputs(config)["bundle_json"]),
        "bundle_markdown_sha256": file_sha256(_repo_outputs(config)["bundle_markdown"]),
