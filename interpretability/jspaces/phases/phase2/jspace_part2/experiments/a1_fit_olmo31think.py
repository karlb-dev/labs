# A1 — fit the 120-prompt J-lens for the PRIMARY checkpoint
...
MERGED = RUN_DIR_P2 / "lens" / "olmo31think_lens.pt"
METRICS = RUN_DIR_P2 / "metrics" / "olmo31-think" / "fit.json"
...
def slice_paths(k: int) -> tuple[Path, Path, Path]:
    return (LOCAL_WORK / f"fit31t_slice{k}.ckpt",
            RUN_DIR_P2 / "lens" / f"fit31t_slice{k}.ckpt",
            RUN_DIR_P2 / "lens" / f"olmo31think_slice{k}.pt")
...
def main():
    ...
    all_slices = sorted(RUN_DIR_P2.glob("lens/olmo31think_slice*.pt"))
    if len(all_slices) < 4:
    ...
    prov = Provenance(
        ...
        command="python -m jspace_part2.experiments.a1_fit_olmo31think",
    ...
    registry_append({
        "evidence_id": "a1-fit-olmo31think-v1", "tier": "pilot",
        "what": (f"PRIMARY-pair lens: Olmo-3.1-32B-Think 120-prompt J-lens "
                 f"(recipe identical to the 3.1-Instruct draw-A fit: "
                 f"{len(SOURCE_LAYERS)} source layers, target "
                 f"{TARGET_LAYER}; closes freeze condition 3)"),
