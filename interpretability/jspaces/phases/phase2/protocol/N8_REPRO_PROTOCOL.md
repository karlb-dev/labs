...
- the `labs` repository checked out at branch `interp_jspace_part2`
  (any commit at or after `interpretability/jspaces/phases/phase2` exposes
  `JSPACE_PART2_RUN_ROOT` / `JSPACE_PART2_OUT_ROOT`; verify with
  `grep -n JSPACE_PART2_OUT_ROOT interpretability/jspaces/phases/phase2/jspace_part2/experiments/confirmatory_analysis.py`)
