...
def _document_rows(documents: dict, *, require: bool = True) -> dict:
    ...
    for name, relative in documents.items():
        ...
        if not path.is_file():
            ...
            continue
        rows[name] = {
            "path": f"interpretability/jspaces/phases/phase4/{relative}",
...
def manifest_stage() -> dict:
    ...
    payload = {
        ...
        "registry": {
            "path": "interpretability/jspaces/phases/phase4/reports/"
        ...
        "fresh_materialization": {
            "artifact": "interpretability/jspaces/phases/phase4/reports/"
