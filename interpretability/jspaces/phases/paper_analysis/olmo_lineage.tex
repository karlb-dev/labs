...
\graphicspath{{figures/}{./}{../phase4/reports/figures/}}
...
\begin{tcolorbox}[colback=red!4!white,colframe=red!55!black,
  title={RETIRED AS A STANDALONE PAPER (2026-08-05, paper-route decision)}]
This draft predates the Study-2 terminal record and is preserved as a
historical snapshot only. The official SFT/DPO wedge is now
\textbf{capability-gated} (972-row batteries; capable rates
0.617\%/0.309\%; zero Bank-S facts capable on direct+composed; route
\texttt{null\_or\_unresolved}), the H6 ladder licenses \textbf{no
in-band finite-dose regime} at L24/L32/L40 (Think-only L56
$\varepsilon{=}0.10$ late anchor), and the Bank-W pair design is
unpowered at current support (0.7788 at 16 shared capable families;
first passing count 18). Current sources of record: the technical
report \texttt{jspace\_olmo\_lineage/reports/paper/olmo\_lineage\_parallel\_phase.tex},
the V2 state of record and claims table, and the empirical paper's
lineage section. See
\texttt{jspace\_paper/analysis/reports/PAPER\_ROUTE\_DECISION.md}.
\end{tcolorbox}
...
Branch snapshot: & \texttt{interp\_jspace\_part2} through 2026-08-01 (pre-Study-2); retired at \texttt{jspace-phase4-frozen-v1}\\
Document version: & August 2026, full lineage rewrite; retired 2026-08-05\\
\end{tabular}
...
package registry is
\path{interpretability/jspaces/sidelines/olmo/reports/evidence_events.jsonl},
