...
%  \eid{...} bracket (registries: interpretability/jspaces/phases/phase3/reports/
%  evidence_events.jsonl and interpretability/jspaces/phases/phase2/reports/
%  evidence_registry.jsonl); numbers were independently re-derived from the
%  item-level parquets by interpretability/jspaces/phases/paper_analysis/scripts/run_analysis.py
%  and re-verified by the paper-analysis reconstruction audit
%  (interpretability/jspaces/phases/paper_analysis/analysis/reports/RECONSTRUCTION_AUDIT.md).
%
...
A preregistered bridge-protection test rejects its null on Qwen
(confirmatory, unreplicated by protocol):
protecting the true intermediate (``bridge'') entity's directions
rescues composed-item damage by $+0.43$ nats relative to protecting a
frozen chosen counterfactual (distractor) bridge, and substituting the
counterfactual bridge's content at matched dose is catastrophically
worse than deletion ($-4.05$ nats, development tier) --- the model
demonstrably reads what sits in the channel, though this route is not
yet separated from a direct answer-direction route.  OLMo shows none of this: its damage is organized by answer
...
\begin{itemize}
  ...
  before any confirmatory outcome was viewed (preregistration
  amendment \texttt{AMENDMENT\_1\_BOS\_UNITS}, not a registry event).
...
Corrected capacity: Qwen at $5$--$6\%$ centered excess, the OLMo 3.1
pair at $0.4$--$1.2\%$ across L24--L40; the cross-paper occupancy
comparison to the reported Claude band remains open (errata W5)
\eid{r2-occupancy-*-v2}.}
...
(historical control realization; state-of-record \texttt{sha256-v1}
realization $-0.271$), exact $p=0.062$ at 17 intersection families;
the wild-cluster percentile-t interval $[-0.537, +0.015]$ crosses
zero --- descriptive and inferentially negative, seed-sensitive across
the five frozen control realizations, under the disclosed MDE
\eid{p3-locked-analysis-v1} \eid{p3-control-seed-contract-audit-v2}.
Replication side $-0.197$, $p=0.219$, same sign.  Think-vs-Instruct
...
protecting the frozen chosen counterfactual (distractor) bridge ($94$
composed items, $p_{\mathrm{holm}}=0.018$; the distractor was chosen,
not randomized, and no held-out-family replication exists by protocol)
\eid{p3-locked-analysis-v1}
...
overlap; span-safety removes $49.3\%$/$71.6\%$/$77.8\%$ of the label
prose cost on Think/Instruct/Qwen respectively.  No model earns
``selective'' wording at this tier
\eid{p3-prose-grid-figure-v1} \tier{phase3-development}.}

%----------------------------------------------------------------------------------------
\section{Reproducibility}\label{sec:repro}

\todo{The N8 ladder as a release gate.  Phase 2: narrative-blind
analysis reproduction to $\le5\times10^{-4}$ worst absolute difference
\eid{p3-n8-level1-repro-v1}.  Phase 3 ladder: Level 1 reproduced all
61/61 quantities exactly \eid{p3-n8-p3-level1-v1}; Level 2 stratified
cells reproduced the deterministic arms on all three models
\eid{p3-n8-p3-level2-*}; Level 3 reproduced one full 188-item Qwen GPU
cell exactly, including the stable control
\eid{p3-n8-p3-level3-qwen36-27b-v1} \tier{methods}.
