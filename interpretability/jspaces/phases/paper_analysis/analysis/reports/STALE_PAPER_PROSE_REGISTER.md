...
| U8 | `jspace_paper/kburtram_jspace.tex:6-8` (header comment) | Points to `interpretability/jspace_runs/analysis/run_analysis.py` | Script lives at `interpretability/jspaces/phases/paper_analysis/scripts/run_analysis.py`; `jspace_runs` is a Drive tarball, not a repo path | Fix provenance comment in P8; verify every quoted number in the P4 reconstruction audit |
