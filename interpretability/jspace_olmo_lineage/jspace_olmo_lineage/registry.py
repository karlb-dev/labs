"""Append-only evidence registry owned only by the OLMo side track."""
...
VALID_EVENTS = {
    "evidence_created", "evidence_imported", "evidence_superseded",
    "evidence_withdrawn", "evidence_corrected",
}
...
def _validate(event: dict) -> None:
    ...
    if event_type == "evidence_created":
        ...
        if event.get("tier") not in NATIVE_TIERS:
            raise RegistryError("import tier must remain development/methods")
        for key in (
        ...
        if not event.get("reason") or not event.get("corrected_fields"):
            raise RegistryError("correction requires reason and fields")
    ...
    with destination.open("a") as handle:
        ...
        try:
            ...
            if stamped["event"] in {"evidence_created", "evidence_imported"}:
                if stamped["evidence_id"] in origins:
                    raise RegistryError(
                        f"duplicate evidence ID {stamped['evidence_id']!r}")
            elif stamped["evidence_id"] not in origins:
                raise RegistryError("status event references unknown evidence")
            if stamped["event"] == "evidence_superseded":
                if stamped.get("superseded_by") not in origins:
                    raise RegistryError("supersession replacement is unknown")
...
def create(evidence_id: str, *, tier: str, what: str, command: str,
           outputs: Iterable[str | Path] = (), inputs: dict | None = None,
           **extra) -> dict:
...
def resolve(evidence_id: str, *, path: str | Path = EVENTS) -> dict:
    ...
    if len(origins) != 1:
        raise RegistryError(
            f"expected one origin for {evidence_id!r}, found {len(origins)}")
    ...
    record["status_events"] = statuses
    record["effective_metadata"] = effective
    record["effective_tier"] = effective["tier"]
    record["superseded_by"] = next((
        row["superseded_by"] for row in reversed(statuses)
        if row["event"] == "evidence_superseded"), None)
    record["withdrawn"] = any(
        row["event"] == "evidence_withdrawn" for row in statuses)
    record["live"] = not record["withdrawn"] and record["superseded_by"] is None
    return record
