...
The evidence progressed in three dated steps. The corrected scale trend, faithful input delivery, OLMo positive control, superposition failure, and cross-corpus instability jointly made a real geometry difference plausible. The exact-JVP validation then ran (\cref{sec:autopsy-ran}): it \emph{operationally} reproduced the mismatch at every tested layer with the OLMo control passing under identical frozen gates --- and, as of 2026-08-02, quarantined itself behind a failed cross-backend parity ceiling. As of 2026-08-03 (Study 2), the target-blind calibrated mixed-precision envelope resolved that quarantine: the preserved discrepancy lies within the frozen ceiling, the selected slot is bit-identical, and the five-layer \texttt{local\_tangent\_mismatch} classifier is licensed as a \emph{closed finite-scale methods result} at tested scope. It remains a measurement, not a mechanism: no named cause is identified, and no nondifferentiability or workspace conclusion follows.
