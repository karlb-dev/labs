...
\section{Own/common lens comparison: repair pending}

The v1 paired analysis is \textbf{\textcolor{coral}{withdrawn}}.
Although the two raw grids pair 252 items and have zero baseline drift,
their scientific seed namespaces differ. Their matched-control random
subspaces therefore differ across frames. For Bank F composed, the
control shift alone is $+0.0616$ nats with its interval above zero, so
the previous frame-contrast intervals and correlations mix lens change
with RNG change and must not be interpreted.
\eid{p4-lens-frame-analysis-olmo3-think-dev-v1 (withdrawn)}

Both raw grids remain valid standalone development evidence:

\begin{center}\small
\begin{tabular}{lrr}
\toprule
cell & own-lens point estimate & common-base-lens point estimate\\
\midrule
...
A common-lens v4 grid is configured with the own-grid namespace. The
hardened analysis now refuses unequal namespaces. No inferential frame
comparison is live until v4 and the replacement v2 analysis pass their
acceptance audits.
...
negative in each standalone lens frame; formal frame stability awaits
the seed-paired repair.
...
estimate.
\item Bank-F standalone point estimates differ by frame, but the prior
paired inference is withdrawn because its controls used different RNG
streams.
...
The pinned base checkpoint is complete and banked. First run the
seed-paired 3.0 common-lens repair and replacement analysis. Next, run
the exact 3.1 Think and 3.1 Instruct checkpoints through the Phase~4
prospective G5 gate and the same seven-condition GPU grid, explicitly
sharing an RNG namespace within each own/common pair. Then synthesize the full base
