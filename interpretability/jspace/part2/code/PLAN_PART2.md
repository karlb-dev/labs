...
## Launch addendum (2026-07-27, VM6 bootstrap — read together with §8)

- **Budget**: user cap ≤200 h campaign wall-clock (2026-07-27 directive); full
  campaign ≈45–55 GPU-h at part-1 rates → execute everything, no thinning.
- **Pins + corrections (hub-checked 2026-07-27):**
  - **A1 = `allenai/Olmo-3.1-32B-Instruct`** — `Olmo-3-32B-Instruct` DOES NOT
    EXIST on the hub. The 3.1 Instruct shares base `allenai/Olmo-3-1125-32B`
    with our Think anchor (model-card base-model table; identical 64L/d5120/
    vocab 100278) → the matched-pretraining pair the plan wants is intact.
    `Olmo-3.1-32B-Instruct-{SFT,DPO}` are public = optional post-training
    trajectory axis if A1 flips anything.
  - **A2**: the Qwen3.6 line ships NO separate non-think dense sibling (only
    `Qwen/Qwen3.6-27B` dense + `Qwen3.6-35B-A3B` MoE). Part 1 already ran the
    27B no-think. A2 re-scoped to completing the same-model matrix: think-mode
    cells (cot-lead + foil floor, rescue mirror), C3 hard one-hop retest of the
    spared-1-hop asymmetry, C2 paper sets, D occupancy, seed-1 + n→60 on the
    frozen grid. Verify hybrid-think support from the chat template at set start.
  - **A3 = `google/gemma-4-31B-it`** (already in the Drive HF cache; no think
    sibling shipped; base `gemma-4-31B` exists if needed).
  - **Optional H1 anchor**: `allenai/Olmo-3-1125-32B` (the shared base) via
    lens transfer, raw-completion cells only — run only if OLMo pass 1 leaves
    slack or an A1 flip makes it decisive (most non-externalizing model in the
    family; the battery is completion-style by design, so it is feasible).
- **Ops grouping (disk residency; does NOT change scientific priority):**
  OLMo pass 1 = A0, B3, A1, C3, C1, D → Qwen set = A2* (+ C2q/C3q/Dq) →
  OLMo pass 2 = B1, C2, B4, B2 (Think is Drive-cached; zero re-download) →
  Gemma = A3 → E (mostly CPU). The §8 A1-flip rule stays live inside pass 1
  (Instruct is still on NVMe exactly when it matters).
- **Storage**: new models → `/content/hf_local` HF cache (measured 320 MB/s
  from the hub — faster than Drive copy), deleted between sets;
  Think/gemma/WikiText served from the Drive cache; never touch DriveFS
  chunks.db (see experiment_reset_instructions.md).
- **Ledger**: SL2-C* claims live in `jspace/part2/README.md` (Lab-36 template)
  until promotion. **Master dataset** `report/matrix_master.{csv,json}` — one
  row per cell {model, ws, instrument, condition, task, dose, n, seed,
  decoding, point, CI, baseline, source file, commit} — is the single source
