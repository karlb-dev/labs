...
| Q | `s18_qwen_instruments.py` (**WRITTEN 2026-07-26 VM4, gated on P1–P5 + GPU window**) | Qwen 3.6 27B leg, pins verified against the hub 2026-07-26: model `Qwen/Qwen3.6-27B` (exists, 15 safetensor shards ≈54GB bf16); lens `neuronpedia/jacobian-lens` file `qwen3.6-27b/jlens/Salesforce-wikitext/Qwen3.6-27B_jacobian_lens_n1000.pt` (3.3GB, 1000-prompt WikiText recipe, trained by @mntss/Anthropic per CREDIT.md; no config.yaml — all-layer lens, `JacobianLens.from_pretrained`). s18 phases: (a) paper multihop sanity via s4.multihop_eval, (b) s5-style descriptive → layer_state_qwen, (c) energy pass + match at k=20 only, (d) 6-condition grid {none, jspace_k20, vmatch_rand_k20, vmatch_nonJ_k20, frozen_j10, frozen_rand10} × s7 battery. Band = OLMo band fractions (31–69% depth) mapped to Qwen layer count at runtime. HF_HUB_CACHE overridden to /content/hf_local (Drive cache can't hold 54GB); ≥60GB free-disk self-check. No-think commensurability: raw completion prompts, no chat template. | v2 metrics/qwen_*.json |
