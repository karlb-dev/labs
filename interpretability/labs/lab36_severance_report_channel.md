...
Done in v4: a B5 sentinel position injects at an upstream interior token and is compared to report-query insertion (`sentinel_content_blind_logit_only`). Done in v5: a sentinel dose sweep (`sentinel_dose_sweep::dose_*`, 2/4/8) and a representational readout probe (`readout_probe`). Done in v6 (maximal-power): the detection set was expanded to 60 heldout items (12/direction), the readout gained a **trained transformed-direction probe** (pooled + per-direction, std nearest-centroid CV, can catch a rotated signal the projection cannot) with report-query positive controls and permutation nulls, and a **sentinel placement sweep** (`sentinel_placement::{early,mid,late}`). A lean `readout` mode runs directions + the probe suite without the generation-heavy verbalized B5 loop.

Across six models the result is a clean, well-powered negative: both positive controls read AUC 1.0, every sentinel readout (projection, trained-pooled, all 5 per-direction) reads chance below its null (~0.63 bar), and verbalized d-prime is 0 at every dose (to 8x) and placement. The content-blind B5 signal is decision-token direct-steering, not propagated monitoring; no recoverable trace reaches the decision token under upstream injection. The remaining open question is reportability of *natural* internal states (not injected directions), which is a different experiment, not a continuation of this test.
