...
| completed gates | P1-G1 bank audit; P1-G2 instrument; P1-G3 validation; freeze (PI-approved); frozen 7B battery; graduation manifest (7B Stop B); DG smoke; 32B replication (Stop C); mechanistic PC (PASS) |
...
| graduated scenarios | 7B: none (Stop B). 32B: `ar_docsection_readme` (−0.438, all ten criteria) → Stop C case study |
| causal result router | 32B Stop C case study: mechanistic PC PASS (addition +0.787 nats p_holm 9e-5; RO transfer +0.93 p_holm 0.016; code-control opposite sign; removal null) — instrument stack validated. Graduated AR scenario: DIRECTION_NOT_IDENTIFIABLE at every frozen depth (val r 0.09–0.13 vs bands 0.40–0.57); no AR causal claim. |
| compute used | ~1 GPU-session: 7B ≈ 25 min batteries+pilots, 32B ≈ 20 min battery + ~10 min mechanism, DG ≈ 3 min, TeX/analysis CPU |
