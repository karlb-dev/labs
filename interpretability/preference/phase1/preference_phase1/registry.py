"""Append-only evidence registry (plan §1.5 schema, jspaces mechanics).
...
def register(
    *,
    ...
    if not event_id.startswith("pref1-"):
        raise RegistryError(f"event_id must start with 'pref1-': {event_id}")
