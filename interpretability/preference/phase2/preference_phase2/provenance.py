...
def env_audit(include_gpu: bool = True) -> dict[str, Any]:
    """Session environment snapshot (addendum C4 gpu_env contract)."""
    audit: dict[str, Any] = {
...
def require_cuda() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            ...
            "model-scale stages (preference_resume.md §2 (unchanged for Phase 2))."
        )


def session_id() -> str:
    return os.environ.get("PREF2_SESSION_ID", "session-" + utc_now())
