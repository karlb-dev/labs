...
mirror_root="${RUNS_MIRROR:-/content/drive/MyDrive/aidataapps/lab03/runs}"
...
jq -n \
  ...
  --arg source "$run_dir" \
  --arg target "$target" \
  --arg mirroredAt "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  '{schemaVersion:1,runId:$runId,source:$source,target:$target,mirroredAt:$mirroredAt}' \
  >"$run_dir/MIRROR_RECEIPT.json"
node --import tsx scripts/archive-run.ts >/dev/null
rsync -a "$run_dir/ARTIFACT_INVENTORY.json" "$run_dir/MIRROR_RECEIPT.json" "$run_dir/RESUME.md" "$target/"
sha256sum "$run_dir/ARTIFACT_INVENTORY.json" "$target/ARTIFACT_INVENTORY.json"
echo "$target"
