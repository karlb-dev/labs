...
docker_user="${LOGWARDEN_DOCKER_USER:-codexdocker}"
docker_uid="${LOGWARDEN_DOCKER_UID:-1000}"
runtime_dir="/run/user/$docker_uid"
docker_socket="unix://$runtime_dir/docker.sock"
daemon_log="${LOGWARDEN_DOCKER_LOG:-/content/aidataapps-logwarden-dockerd.log}"

# shellcheck disable=SC1091
source /etc/os-release
if [[ "${ID:-}" != "ubuntu" ]]; then
  echo "This bootstrap supports Ubuntu Colab runtimes, not ${ID:-unknown}." >&2
  exit 1
fi

apt-get update
apt-get install -y ca-certificates curl fuse-overlayfs gnupg jq openssl rsync slirp4netns tmux uidmap
