...
try {
  ...
  const observed = await pool.request().input("profile", sql.VarChar(80), profileKey)
    ...
    .query<Record<string, unknown>>(`
      ...
      (
        SELECT prediction.prediction_id,prediction.job_id,prediction.agent_run_id,prediction.agent_arm_id
        FROM eval.predictions prediction
        INNER JOIN eval.ground_truth_episodes truth ON truth.episode_id=prediction.episode_id
        WHERE prediction.model_profile_id=@profile AND prediction.control_id IS NULL
          AND truth.split_role='dev' AND prediction.agent_run_id IS NOT NULL
      )
      SELECT
        (SELECT COUNT(*) FROM current_predictions WHERE agent_arm_id='A-direct') direct_predictions,
        (SELECT COUNT(*) FROM current_predictions WHERE agent_arm_id='A-tools') tools_predictions,
        (SELECT COUNT(*) FROM current_predictions WHERE agent_arm_id='A-rag') rag_predictions,
        ...
        (SELECT COUNT(*) FROM agent.agent_runs agent_run INNER JOIN current_predictions prediction ON prediction.agent_run_id=agent_run.agent_run_id) agent_runs,
        (SELECT COUNT(*) FROM agent.agent_runs agent_run INNER JOIN current_predictions prediction ON prediction.agent_run_id=agent_run.agent_run_id WHERE agent_run.status='running') open_agent_runs,
        (SELECT COUNT(*) FROM agent.turns turn INNER JOIN current_predictions prediction ON prediction.agent_run_id=turn.agent_run_id) turns,
        (SELECT COUNT(*) FROM agent.model_requests request INNER JOIN agent.turns turn ON turn.turn_id=request.turn_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=turn.agent_run_id) requests,
        (SELECT COUNT(*) FROM agent.model_responses response INNER JOIN agent.model_requests request ON request.model_request_id=response.model_request_id INNER JOIN agent.turns turn ON turn.turn_id=request.turn_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=turn.agent_run_id) responses,
        (SELECT COUNT(*) FROM agent.model_responses response INNER JOIN agent.model_requests request ON request.model_request_id=response.model_request_id INNER JOIN agent.turns turn ON turn.turn_id=request.turn_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=turn.agent_run_id WHERE response.prompt_tokens IS NULL OR response.completion_tokens IS NULL OR response.total_tokens IS NULL) missing_token_rows,
        (SELECT COUNT(*) FROM agent.model_responses response INNER JOIN agent.model_requests request ON request.model_request_id=response.model_request_id INNER JOIN agent.turns turn ON turn.turn_id=request.turn_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=turn.agent_run_id WHERE (response.reasoning_content IS NULL AND (response.reasoning_sha256 IS NOT NULL OR response.reasoning_bytes IS NOT NULL)) OR (response.reasoning_content IS NOT NULL AND (response.reasoning_sha256 IS NULL OR response.reasoning_bytes IS NULL))) reasoning_provenance_failures,
        (SELECT COUNT(*) FROM agent.tool_invocations tool INNER JOIN current_predictions prediction ON prediction.agent_run_id=tool.agent_run_id) tool_calls,
        (SELECT COUNT(*) FROM agent.tool_invocations tool INNER JOIN current_predictions prediction ON prediction.agent_run_id=tool.agent_run_id WHERE tool.retrieval_run_id IS NOT NULL) retrieval_tool_calls,
        (SELECT COUNT(*) FROM telemetry.spans span INNER JOIN agent.agent_runs agent_run ON agent_run.trace_id=span.trace_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=agent_run.agent_run_id WHERE span.span_name='model.request' AND span.model_request_id IS NULL) unlinked_model_spans,
        (SELECT COUNT(*) FROM telemetry.spans span INNER JOIN agent.agent_runs agent_run ON agent_run.trace_id=span.trace_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=agent_run.agent_run_id WHERE span.span_name='tool.operation' AND span.tool_invocation_id IS NULL) unlinked_tool_spans,
        (SELECT COUNT(*) FROM telemetry.spans span INNER JOIN agent.agent_runs agent_run ON agent_run.trace_id=span.trace_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=agent_run.agent_run_id WHERE span.finished_at_utc IS NULL) open_spans,
        (SELECT COUNT(*) FROM telemetry.journal_events event INNER JOIN agent.agent_runs agent_run ON agent_run.trace_id=event.trace_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=agent_run.agent_run_id) journal_events,
        ...
        (SELECT COUNT(*) FROM ops.action_proposals proposal INNER JOIN agent.decisions decision ON decision.decision_id=proposal.decision_id INNER JOIN current_predictions prediction ON prediction.agent_run_id=decision.agent_run_id WHERE proposal.caller_opted_in=1 OR proposal.executed_at_utc IS NOT NULL OR proposal.execution_status<>'not_executed') unsafe_actions;
    `);
