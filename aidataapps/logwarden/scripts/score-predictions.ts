...
try {
  ...
  for (const prediction of predictions) {
    ...
    insertedRetrievalScores += await persistRetrievalScore(
      predictionId,
      returnedRows[0]?.retrieval_run_id ?? null,
      retrieval,
  ...
  const result = await pool.request().input("roles", sql.NVarChar(sql.MAX), JSON.stringify(roles))
    ...
    .query<PredictionRow>(`
      ...
      SELECT prediction.prediction_id,prediction.job_id,prediction.agent_run_id,prediction.episode_id,
        ...
        truth.protected_truth_json,decision.contract_valid,decision.policy_valid
...
async function loadTools(ids: string): Promise<ToolRow[]> {
  const result = await pool.request().input("ids", sql.NVarChar(sql.MAX), ids).query<ToolRow>(`
    ...
    SELECT CONVERT(varchar(30),prediction.prediction_id) prediction_id,tool.tool_name,tool.status,tool.policy_status,tool.snapshot_miss
    ...
    ORDER BY prediction.prediction_id,tool.tool_invocation_id;
  `);
  return result.recordset;
}
...
async function loadReturnedRunbooks(ids: string): Promise<RunbookRow[]> {
  const result = await pool.request().input("ids", sql.NVarChar(sql.MAX), ids).query<RunbookRow>(`
    ...
    WHERE result.returned_to_agent=1 ORDER BY prediction.prediction_id,tool.tool_invocation_id,result.rank_ordinal;
  `);
  return result.recordset;
}
