...
BEGIN
  ...
  (
    ...
    WHERE s.enabled=1 AND (@scenario_role_filter IS NULL OR v.split_role=@scenario_role_filter)
  ...
  (
    SELECT 1 AS schemaVersion, @seed AS seed, @scenario_role_filter AS scenarioRoleFilter,
           @repeat_count AS repeatCount, @rate_profile AS rateProfile,
           CASE WHEN @rate_profile='capture-gapped' THEN 121000
                WHEN @rate_profile='storm' THEN 100 ELSE 2500 END AS interGroupGapMs,
  ...
  (
    SELECT v.scenario_variant_id, v.variant_group_id, v.split_role,
           DENSE_RANK() OVER
           (
             ORDER BY HASHBYTES('SHA2_256', CONCAT(@seed, N':', v.variant_group_id)), v.variant_group_id
           ) - 1 AS group_ordinal
    FROM workload.scenario_variants AS v
    INNER JOIN workload.scenario_definitions AS s ON s.scenario_id=v.scenario_id
    WHERE s.enabled=1 AND (@scenario_role_filter IS NULL OR v.split_role=@scenario_role_filter)
  ...
  (
    SELECT v.*, series.value AS repeat_ordinal,
           HASHBYTES('SHA2_256', CONCAT(@seed, N':', v.scenario_variant_id, N':', series.value)) AS order_hash
    FROM variants AS v CROSS JOIN GENERATE_SERIES(0, @repeat_count-1, 1) AS series
  ), planned AS
  (
    SELECT *,
      CASE WHEN @rate_profile='capture-gapped' THEN group_ordinal*CONVERT(bigint,121000) + repeat_ordinal*1000
           WHEN @rate_profile='storm' THEN ROW_NUMBER() OVER(ORDER BY group_ordinal, repeat_ordinal, order_hash)*CONVERT(bigint,100)
           ELSE ROW_NUMBER() OVER(ORDER BY group_ordinal, repeat_ordinal, order_hash)*CONVERT(bigint,2500)
