...
BEGIN
  ...
  (
    SELECT
      ...
      TRY_CONVERT(int, NULLIF(x.event_data.value('(/event/data[@name="error_number"]/value/text())[1]', 'nvarchar(100)'), N'')) AS error_number,
      TRY_CONVERT(int, NULLIF(x.event_data.value('(/event/data[@name="severity"]/value/text())[1]', 'nvarchar(100)'), N'')) AS severity,
      TRY_CONVERT(int, NULLIF(x.event_data.value('(/event/data[@name="state"]/value/text())[1]', 'nvarchar(100)'), N'')) AS error_state,
      x.event_data.value('(/event/data[@name="message"]/value/text())[1]', 'nvarchar(4000)') AS message_raw,
      TRY_CONVERT(bigint, NULLIF(x.event_data.value('(/event/data[@name="duration"]/value/text())[1]', 'nvarchar(100)'), N'')) AS duration_microseconds,
      TRY_CONVERT(bigint, NULLIF(x.event_data.value('(/event/data[@name="cpu_time"]/value/text())[1]', 'nvarchar(100)'), N'')) AS cpu_microseconds,
      TRY_CONVERT(bigint, NULLIF(x.event_data.value('(/event/data[@name="logical_reads"]/value/text())[1]', 'nvarchar(100)'), N'')) AS logical_reads,
      TRY_CONVERT(bigint, NULLIF(x.event_data.value('(/event/data[@name="writes"]/value/text())[1]', 'nvarchar(100)'), N'')) AS writes,
      x.event_data.value('(/event/action[@name="database_name"]/value/text())[1]', 'nvarchar(128)') AS database_name,
      TRY_CONVERT(int, NULLIF(x.event_data.value('(/event/action[@name="session_id"]/value/text())[1]', 'nvarchar(100)'), N'')) AS session_id,
