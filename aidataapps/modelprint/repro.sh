...
local_backup="$(find "$run_dir/database" -maxdepth 1 -type f -name '*.bak' -printf '%T@ %p\n' | sort -nr | head -1 | cut -d' ' -f2-)"
...
if [[ -d "$mirror_run/database" ]]; then
  mirror_backup="$(find "$mirror_run/database" -maxdepth 1 -type f -name '*.bak' -printf '%T@ %p\n' | sort -nr | head -1 | cut -d' ' -f2-)"
fi
...
if [[ -n "$mirror_backup" && -f "$mirror_backup.sha256" ]]; then
  ...
  mirror_actual="$(sha256sum "$mirror_backup" | awk '{print $1}')"
...
[[ -n "$backup_sha" ]] || backup_sha="$(sha256sum "$backup" | awk '{print $1}')"
...
jq -n --arg runId "$run_id" --arg startedAt "$started_at" --arg completedAt "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  --arg gitHead "$(git rev-parse HEAD)" --arg inventorySha256 "$(sha256sum "$inventory" | awk '{print $1}')" \
  --arg backupSource "$backup_source" --arg backupFile "$(basename "$backup")" --arg backupSha256 "$backup_sha" \
  --arg headlineSha256 "$(sha256sum "$run_dir/reports/HEADLINE.md" | awk '{print $1}')" \
