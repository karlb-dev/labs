...
<!-- generated by analysis/build_reports.py -->
The strongest held-family minimum was `likelihood-profile8-v1` at macro-F1 `0.958` with taxonomy `CLOSED_SET_SIGNAL`.
Across matched suites, the best text-only probe reached `0.640` of the likelihood-profile macro-F1 on average.
ANN disposition: semantic1024-segment-union-v1: ANN_DISTORTS; fingerprint64-v1: ANN_UNNEEDED_AT_SCALE.
Legitimate app statement: for sufficiently long text from the frozen four served profiles, the app can return scoped witnesses and a calibrated closed-set candidate or abstain; it cannot prove authorship of arbitrary prose.
