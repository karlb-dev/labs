...
[[ "$(stat -c %s "$drive_backup")" == "$(stat -c %s "$latest_backup")" && "$(sha256_file "$drive_backup")" == "$backup_sha" ]] || { echo "Drive native backup size/digest mismatch." >&2; exit 2; }
[[ "$(awk 'NR==1 {print $1}' "$drive_backup.sha256")" == "$backup_sha" && "$(jq -r '.sha256' "$drive_backup.json")" == "$backup_sha" ]] || { echo "Drive native backup metadata mismatch." >&2; exit 2; }
...
if [[ -n "$latest_bacpac" && -f "$latest_bacpac" ]]; then
  ...
  [[ "$(stat -c %s "$drive_bacpac")" == "$bacpac_bytes" && "$(sha256_file "$drive_bacpac")" == "$bacpac_sha" ]] || { echo "Drive BACPAC size/digest mismatch." >&2; exit 2; }
  [[ "$(awk 'NR==1 {print $1}' "$drive_bacpac.sha256")" == "$bacpac_sha" && "$(jq -r '.sha256' "$drive_bacpac.json")" == "$bacpac_sha" ]] || { echo "Drive BACPAC metadata mismatch." >&2; exit 2; }
fi
jq -n \
