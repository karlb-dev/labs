...
# Mac host bootstrap for the GhostType mac profile (see docs/MAC_PROFILE.md).
...
runs_mirror="${LOGWARDEN_RUNS_MIRROR:-$HOME/aidataapps/lab04/runs}"
mkdir -p "$runs_mirror"

if [[ ! -f .env ]]; then
  umask 077
  sa_password="GhostType!$(openssl rand -hex 18)"
  lab_password="GtLab!$(openssl rand -hex 18)"
  service_password="GtService!$(openssl rand -hex 18)"
  sed \
    -e "s/replace-with-a-generated-strong-password/$sa_password/" \
    -e "s/replace-with-a-generated-lab-password/$lab_password/" \
    -e "s/replace-with-a-generated-service-password/$service_password/" \
